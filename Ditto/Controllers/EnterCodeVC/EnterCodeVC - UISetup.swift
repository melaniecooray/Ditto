...
extension EnterCodeViewController {
    func setUpLabels() {
        ...
        tagLabel.center = CGPoint(x: view.frame.width/2, y: view.frame.height/5)
        tagLabel.textAlignment = .center
        tagLabel.text = "find a playlist"
        tagLabel.font = UIFont(name: "Roboto-Light", size: 28)
        view.addSubview(tagLabel)
    }
    
    func setUpInteractive() {
        codeInput = UITextField(frame: CGRect(x: 0, y: 0, width: view.frame.width * 2/3, height: view.frame.height/11))
        codeInput.center = CGPoint(x: view.frame.width/2, y: view.frame.height/3)
        ...
        searchButton = UIButton(frame: CGRect(x: 0, y: 0, width: view.frame.width/3, height: view.frame.height/22))
        searchButton.center = CGPoint(x: view.frame.width/2, y: view.frame.height/2)
        searchButton.setTitle("Search", for: .normal)
        searchButton.setTitleColor(.white, for: .normal)
        searchButton.backgroundColor = UIColor(hexString: "#7383C5")
        searchButton.titleLabel!.font = UIFont(name: "Roboto-Bold", size: 20)
